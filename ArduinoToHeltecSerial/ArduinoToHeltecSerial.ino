...
SoftwareSerial SerialToESP(31, 30); // le pongo los pines que quiero
...
void setup() {
  ...
  if (Ethernet.hardwareStatus() == EthernetNoHardware) { //Ver que esté físicamente conectado 
    Serial.println("W5100 not found!");
  }
  
  if (Ethernet.linkStatus() == LinkOFF) { //ver que haya enlace con el cable de red
...
void loop() { //aca vemos si esta conectado, si no, intentamos conectar

/*
 * CODIGO 2: HELTEC V3.3 (Transmisor LoRa P2P)
 * -------------------------------------------
 * Escucha en Serial1 por un byte del Arduino.
 * Cuando lo recibe, lo transmite usando LoRa P2P.
 * Utiliza la librería Heltec "LoRaWan_APP.h".
 */

#include "LoRaWan_APP.h"
#include "Arduino.h"

// --- Configuración de Radio (TU RESPONSABILIDAD) ---
// Define tu frecuencia. ¡Debe ser la misma en el TX y RX!
// Ej: 915E6 (915 MHz) o 868E6 (868 MHz)
...
void setup() {
  ...
  // ¡Debe ser el mismo baud rate (9600)!
  // ESP32-S3 (Heltec V3) pines por defecto para Serial1:
  // RX = 47, TX = 48
  // Asegúrate de que estos son los pines que conectaste al Arduino
  ...
  Radio.SetTxConfig(MODEM_LORA, txPower, 0, bandwidth,
                    ...
                    preambleLength, false, crc,
                    0, 0, false, 3000); // Timeout de 3 seg

  // Le dice a la librería que no estamos usando una red pública LoRaWAN
  Radio.SetPublicNetwork(false); 
  
  Serial.println("Radio TX lista. Esperando byte del Arduino...");
}
...
void loop() {
  ...
  if (txIniciada == false) {
    // Revisa si hay un byte disponible desde el Arduino
    if (Serial1.available() > 0) {
      byte byteDelArduino = Serial1.read();
      
      Serial.print("Byte recibido de Arduino: 0b");
